...
// X30 adds a maximum angle between the candidate plane and the floor.
...
tryPoint(const Eigen::Vector3f& iPoint, const Eigen::Vector3f& iNormal,
         ...
         const float iMaxAngleToFloor) {
  // The X30 factory implementation keeps these arguments in its ABI but
  // overwrites both before use. Preserve that observed behavior exactly.
  (void)iNormal;
  (void)iMaxAngle;
  ...
  Eigen::Matrix3d sumSquared = mSumSquared + p*p.transpose();
  Eigen::Vector4f plane = getPlane(sum, sumSquared, n+1);
  if (std::abs(plane[2]) < std::cos(iMaxAngleToFloor)) return false;
