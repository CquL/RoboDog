...
// Derived from ori-drs/plane_seg at the commit pinned in third_party/plane_seg.
// X30-specific deltas are kept in this file; do not edit the pinned snapshot.
...
void BlockFitter::
setComputeVerticalPlane(const bool iComputeVerticalPlane) {
  mComputeVerticalPlane = iComputeVerticalPlane;
}

void BlockFitter::
setMinPoint(const int iMinPoint) {
  mMinPoint = std::max(3, iMinPoint);
}
...
go() {
  ...
  // The recovered factory binary always takes this horizontal-plane path.
  // setComputeVerticalPlane() exists in its ABI but go() never reads the flag.
  ...
  // Values recovered from the X30 AArch64 BlockFitter::go(). In this factory
  // build max-angle is retained in the ABI but the five-argument tryPoint()
  // only enforces maxAngleToFloor against global Z.
  segmenter.setMaxError(0.025F);
  segmenter.setMaxAngle(15.0F);
  segmenter.setMaxAngleToFloor(15.0F);
  segmenter.setSearchRadius(0.08F);
  segmenter.setMinPoints(mMinPoint);
