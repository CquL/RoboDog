...
static bool has_valid_quaternion(const quat_data_t &quat)
{
	const double norm =
		quat.q0 * quat.q0 +
		quat.q1 * quat.q1 +
...
static geometry_msgs::msg::Quaternion quaternion_from_euler_deg(float roll_deg, float pitch_deg, float yaw_deg)
{
	const double roll = roll_deg / DEG_TO_RAD_FACTOR;
	const double pitch = pitch_deg / DEG_TO_RAD_FACTOR;
	const double yaw = yaw_deg / DEG_TO_RAD_FACTOR;
...
class YESENSE_Publisher : public rclcpp::Node
{
	...
	: Node("yesense_publisher")
	{
		...
		this->declare_parameter<std::string>("imu_topic_ros", "/x30/body_imu");
		this->declare_parameter<std::string>("imu_topic", "/x30/body_imu_raw");
		...
		*((unsigned int *)&yis_out) = 0u;		
		...
		if(0 == memcmp(driver_type_str.c_str(), "linux_serial", strlen("linux_serial")))
		{
		...
		{		
			...
			{				
				...
				ser.setParity(serial::parity_t::parity_none);       //璁剧疆鏍￠獙浣?
	...
	int fd;				// linux椹卞姩涓嬬殑涓插彛

	// configuration
	std::string serial_port;
	int baud_rate;
	std::string frame_id;	
	int driver_type;	// 閫夋嫨浣跨敤ROS涓插彛椹卞姩鎴栨槸linux鍘熺敓椹卞姩
...
void YESENSE_Publisher::publish_msg(yis_out_data_t *result)
{
	...
	if(has_valid_quaternion(result->quat))
	{
	...
	else if(result->content.euler)
	{
		imu_ros_data.orientation = quaternion_from_euler_deg(
			result->euler.roll,
			result->euler.pitch,
			result->euler.yaw);
	}
