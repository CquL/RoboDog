...
// X30 adds an explicit plane-angle-to-floor constraint.
...
PlaneSegmenter() {
  setMaxError(0.02);
  setMaxAngle(5);
  // The factory constructor leaves this field uninitialized. Initialize it to
  // the value explicitly supplied by BlockFitter::go() to remove undefined
  // behavior without claiming it was a recovered constructor default.
  setMaxAngleToFloor(15);
  setSearchRadius(0.1);
  setMinPoints(500);
}
...
void PlaneSegmenter::
setMaxAngleToFloor(const float iAngleToFloor) {
  mMaxAngleToFloor = iAngleToFloor*M_PI/180;
}
...
go() {
  ...
  auto processPoint = [&](const int iIndex) {
    ...
    const Eigen::Vector3f norm(cloudNorm.normal_x, cloudNorm.normal_y,
                               cloudNorm.normal_z);
    if (planeEst.tryPoint(
        pt, norm, mMaxError, mMaxAngle, mMaxAngleToFloor)) {
