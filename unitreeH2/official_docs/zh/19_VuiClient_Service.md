**介绍**
H2上半身内嵌的音频/灯光相关的硬件设施，使得机器人本体具备了实现语音交互的能力,包括:

- RGB灯条 - 256色
- 扬声器 - 8Ω 10瓦(峰值12瓦)
- 四麦-麦克风阵列，麦间距30mm(类线性,硅麦)

**二次开发**
软件服务版本要求：
vui_service >= 2.3.0.1，vui module >= 2.0.0.3。若内置服务版本不符合此要求，请联系技术支持升级至正确版本。
...
| text | 字符串 | 语音识别结果 |
| angle | 整型 | 方位角度 0-180 |
| speaker_id | 整型 | 说话人识别结果 |
| sense | 字符串 | 情绪识别结果 |
| confidence | 浮点型 | 置信度 |
| language | 字符串 | 语言种类 |
| is_final | 整型 | 结束标志位 (用于流式识别模式，默认非流式) |
