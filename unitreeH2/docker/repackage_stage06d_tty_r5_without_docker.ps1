...
if (-not $SourceBundle) {
    $SourceBundle = Join-Path $unitreeH2Root `
        "runtime_bundle\unitree_h2_pc2_native_amd64_stage06c_to_06e_20260716_r4.tar.gz"
}
...
parent_name="unitree_h2_pc2_native_amd64_stage06c_to_06e_20260716_r4"
...
parent_sha256="$(sha256sum "$source_bundle" | awk '{print $1}')"
{
  ...
  printf 'repackage_reason=stage06d_tty_fd3_fix\n'
...
# These payload trees must remain byte-for-byte unchanged from r4.
...
printf 'R5_PARENT_SHA256=%s\n' "$parent_sha256"
printf 'R5_BUNDLE_PATH=%s\n' "$archive"
printf 'R5_BUNDLE_SHA256=%s\n' "$(awk '{print $1}' "$archive_hash")"
...
Write-Host "R5_BUNDLE_PATH=$archivePath"
Write-Host "R5_BUNDLE_SHA256=$hash"
Write-Host "R5_TTY_FIX_REPACKAGE_HOST_OK"
